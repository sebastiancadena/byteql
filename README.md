...
ByteQL turns record-oriented binary files (MIDI, pcap/pcapng, and ZIP today; evtx, regf, and more
next)
...
| `packages/formats/pcap` | `@byteql/pcap` | Network capture pack: classic pcap and pcapng containers, 12-parser dissect chain (ethernet/Linux SLL/SLL2 → ipv4/ipv6 → tcp/udp → dns/icmp/icmpv6/tls) with TCP stream reassembly.                                                                                         |
